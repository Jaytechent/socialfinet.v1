...
const handlePostContentToTwitter = async (contentToPost) => {
  try {
    const result = await userClient.v2.tweet(contentToPost);
    console.log(`${chalk.bgBlack.bold("Posted successfully to X:")}`, result);
  } catch (error) {
    console.error(`${chalk.bgRed.bold("Error occurred while posting to X:")}`, error);
  }
};

export { userClient, handlePostContentToTwitter };
